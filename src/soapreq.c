...
static int registerdevice(xmlNodePtr registerdevice, DevicePtr dev) {
	xmlNodePtr
	deviceID = xmlNewNode(NULL, BAD_CAST "DeviceID");
...
xmlNodePtr
selectfields = xmlNewNode(NULL, BAD_CAST "SelectFields");
xmlAddChild(customsearch, selectfields);
xmlNodePtr selectfield = NULL;
int i;
const char *devParams[DEV_PARAM_SIZE] = { "OUI", "SerialNumber", "ProductClass",
		"Description" };
...
xmlNodePtr
criteria = xmlNewNode(NULL, BAD_CAST "Criteria");
...
xmlNsPtr
nssoapenv = xmlNewNs(envelope, BAD_CAST"http://schemas.xmlsoap.org/soap/envelope/", BAD_CAST "soapenv");
...
xmlNsPtr
nsnbi = xmlNewNs(envelope, BAD_CAST "urn:www.acslite.com/nbi:1.4", BAD_CAST "nbi");
xmlNodePtr
sd = xmlNewNode(nsnbi, BAD_CAST "CustomSearch");
customsearch(sd, dev);
if (sd)
xmlAddChild(body, sd);
else
...
nsnbi = xmlNewNs(envelope, BAD_CAST "urn:www.acslite.com/nbi:1.4", BAD_CAST "nbi");
xmlNodePtr
sd = xmlNewNode(nsnbi, BAD_CAST "RegisterDevice");
...
nsnbi = xmlNewNs(envelope, BAD_CAST "urn:www.acslite.com/nbi:1.4", BAD_CAST "nbi");
xmlNodePtr
sd = xmlNewNode(nsnbi, BAD_CAST "DeleteDevice");
deletedevice(sd, dev);
xmlAddChild(body, sd);
}
